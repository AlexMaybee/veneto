...
//mycode
//функция для получения кода товара по имени товара
function getProductCode($productID){
    $res = '';
    $filter = array('ID' => $productID, 'IBLOCK_ID' => 20);
    $select = array('ID','NAME','CATALOG_QUANTITY');
    $massive = CIBlockElement::GetList(Array(), $filter, false, false, $select);
    while($products = $massive->getNext()){
        $res = $products;
    }
    return $res;
}